    ...
    <div className="page assistant-page">
      <section className="assistant-shell">
        ...
        <section className="assistant-question" aria-live="polite">
          ...
          ) : <div className="preference-orbit" aria-label="Travel preferences">{preferenceOptions.map((option) => <button key={option.id} type="button" className={selectedPreferences.includes(option.id) ? 'preference-chip preference-chip--selected' : 'preference-chip'} onClick={() => togglePreference(option)} aria-pressed={selectedPreferences.includes(option.id)}>{selectedPreferences.includes(option.id) && <Check size={14} />}{option.label}</button>)}</div>}
